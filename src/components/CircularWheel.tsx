...
const CircularWheel = () => {
  ...
  const spinWheel = () => {
    ...
    const finalRotation =
      numRotations * 360 + randomSegment * segmentSize + randomOffset;

    const winningIndex =
      items.length - Math.floor((finalRotation % 360) / segmentSize) - 1;
  ...
  return (
    <>
      <div className="max-w-4xl mx-auto block p-20 border backdrop-blur-lg bg-white/3 rounded-lg shadow-[6px_3px_26px_0px_rgba(0,_0,_0,_0.1)]">
          ...
          <div className="flex-1 relative">
            <div className="relative w-96 h-96 mx-auto">
              ...
              >
                <svg
                  viewBox="-1 -1 2 2"
                  className="items w-full h-full transform -rotate-90"
                >
                  {items.map((item, i) => {
      ...
      {/* Result Modal */}
      {showModal && <ResultModal winner={winner} resetGame={resetGame} />}
    </>
  );
};
